...
def running_average_loss(model: nn.Module, running_avg: list) -> torch.FloatTensor:
    ...
    running_avg_loss = [
        (p - r).pow(2).mean() for p, r in zip(model.parameters(), running_avg)
    ]
    running_avg_loss = sum(running_avg_loss) / len(running_avg_loss)
    return running_avg_loss


def eval_critic_batch(real: torch.FloatTensor, config: dict) -> dict:
    ...
    errD_real = loss(predicted, labels)
    ...
    metrics["D_loss"] = (
        errD + config["weight_avg_factor_d"] * metrics["running_avg_loss_D"]
    )
...
def eval_generator_batch(
    ...
    metrics["G_loss"] = (
        errG + config["weight_avg_factor_g"] * metrics["running_avg_loss_G"]
    )
